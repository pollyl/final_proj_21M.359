...
class UIGraphics(InstructionGroup):
   def __init__(self):
      ...
      # record button and instrument select box
      self.record_circle_color = Color(*(1, 1, 1), mode='hsv')
      self.record_circle = Ellipse(segments = 40, pos = (650, 50), size=(110, 110))
      self.instrument_select_points = [40, 40, 180, 40, 180, 180, 40, 180, 40, 40]
      self.instrument_select =  Line(points=self.instrument_select_points, width=5)

      # instruments
      self.instrument_color = Color(*(0, 0, 1), mode='hsv')
      self.guitar = Rectangle(source="./images/guitar.png", pos=(50, 50), size=(120, 120))
      self.keyboard = Rectangle(source="./images/keyboard.png", pos=(200, 50), size=(120, 120))
      self.drum = Rectangle(source="./images/drum.png", pos=(350, 50), size=(120, 120))
      self.bass = Rectangle(source="./images/bass.png", pos=(500, 50), size=(120, 120))

   def show(self):
      self.add(self.record_circle_color)
      self.add(self.instrument_select)
      self.add(self.record_circle)
      self.add(self.instrument_color)
      self.add(self.guitar)
   ...
   def hide(self):
      ...
      self.remove(self.instrument_select)
      self.remove(self.instrument_color)
      self.remove(self.keyboard)
      self.remove(self.guitar)
      self.remove(self.keyboard)
      self.remove(self.drum)
      self.remove(self.bass)
